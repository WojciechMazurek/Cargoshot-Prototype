...
//
//  Created by Wojciech Mazurek
//  Parts taken from Nicholas Arner as well as other Roboflow documentations and examples
//
...
var API_KEY = "vP3rorc3OpYp50gi1bq9"
...
//    //Initialize the Roboflow SDK
    let rf = RoboflowMobile(apiKey: "vP3rorc3OpYp50gi1bq9")
...
//
    override func viewDidLoad() {
        ...
        loadRoboflowModelWith(model: "cvcargoshot", version: 1 , threshold: 0.1, overlap: 0.2, maxObjects: 2)
        ...
        roboflowModel?.detect(pixelBuffer: pixelBuffer, completion: { detections, error in
            if error != nil {
                ...
                DispatchQueue.main.async { [self] in
                    ...
                    //update and display the height
                    let heightOfCargo = calcHeight(detections: detectionResults)
                    let StringHeight = "Height: " + String(heightOfCargo)
                    if (heightOfCargo != -1){
                        heightLabel.text = StringHeight
                    }
                    //update and display the pallet type
                    if (detectionResults.isEmpty){
                        palletLabel.text = "Nothing Found"
                    }
                    
                    for detection in detectionResults {
                        let detectionInfo = detection.vals()
                        if (detectionInfo["class"] as! String ==  "White Pallet"){
                            palletLabel.text = "White Pallet Found"
//                            break so that no other label can be detected, should not be an issue, since only one type of pallet should be detected.
                            break
                        }
                        else if(detectionInfo["class"] as! String ==  "Black Pallet"){
                            palletLabel.text = "Black Pallet Found"
                            break
                        }
                        else if(detectionInfo["class"] as! String ==  "Cargo"){
                            palletLabel.text = "Only Cargo Found"
                        }
                    }
                }
            }
    ...
    func calcHeight(detections: [RFObjectDetectionPrediction])-> Float{
        var cargoHeight:Float = -1
        var palletHeight:Float = -1
        for detection in detections {
            let detectionInfo = detection.vals()
            if (detectionInfo["class"] as! String ==  "Cargo"){
                cargoHeight = detectionInfo["height"] as! Float
            }
            if (detectionInfo["class"] as! String ==  "White Pallet"){
                palletHeight = detectionInfo["height"] as! Float
            }
        }
        if cargoHeight == -1 || palletHeight == -1{
            return -1
        }
        else {
            let pxlsForInch: Float = palletHeight/4.5
            let CargoHeightInchs: Float  = cargoHeight/pxlsForInch
            return CargoHeightInchs
        }
    }
    ...
    @IBAction func uploadImage(_ sender: Any) {
        ...
        let alert = UIAlertController(title: "Incorrect Pallet?", message: "You've captured an image of a wrong pallet type. Upload it to the open source dataset to improve this model.", preferredStyle: .alert)
...
//    @IBAction func upload2read() {
//        let project = "cvcargoshot"
//        
//        rf.load(model: "cvcargoshot", modelVersion: 1) { [self] model, error, modelName, modelType in
//            if error != nil {
//                print(error?.localizedDescription as Any)
//            } else {
//                model?.configure(threshold: 0.1, overlap: 0.2, maxObjects: 2)
//                self.model = model
//            }
//        }
//    }
